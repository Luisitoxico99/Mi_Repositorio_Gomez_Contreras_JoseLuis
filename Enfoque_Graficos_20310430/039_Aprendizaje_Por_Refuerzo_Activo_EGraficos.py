...
class ActiveReinforcementLearning:
    def __init__(self, num_states, num_actions, learning_rate=0.1, discount_factor=0.9, epsilon=0.1):
        ...
        self.values = np.zeros((num_states, num_actions))  # Función de valor inicializada a cero
    ...
    def choose_action(self, state):
        ...
        if np.random.rand() < self.epsilon:
            ...
            return np.argmax(self.values[state])  # Explotación de la mejor acción conocida

    def update(self, state, action, reward, next_state):
        # Actualización de la función de valor utilizando la regla de actualización de Q-learning
        target = reward + self.discount_factor * np.max(self.values[next_state])
...
# Simulación de episodios de interacción y aprendizaje activo
def simulate_episodes(env, agent, num_episodes):
    for _ in range(num_episodes):
        ...
        while True:
            ...
            agent.update(state, action, reward, next_state)  # Actualizar la función de valor
...
# Crear el agente de aprendizaje por refuerzo activo
agent = ActiveReinforcementLearning(num_states, num_actions, learning_rate, discount_factor, epsilon)
